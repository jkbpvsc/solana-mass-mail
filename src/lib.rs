...
pub mod nonblocking {
    ...
    pub async fn dryrun<T: Signers + Clone>(
        rpc_client: &RpcClient,
        tx_builders: Vec<MailBuilder<T>>,
    ) -> Result<(), MassmailError> {
        let start = Instant::now();
    ...
    pub async fn massmail<T: Signers + Clone>(
        rpc_client: &RpcClient,
        mut outstanding_to_send: Vec<MailBuilder<T>>,
        cfg: MassmailCfg,
    ) -> Result<(), MassmailError> {
        ...
        while landed.len() != total_to_send {
            ...
            blockhash = rpc_client.get_latest_blockhash().await.unwrap();
            ...
            for (sig, (tx_builder, blockhash, send_time)) in to_confirm.iter() {
                ...
                let status = rpc_client
                    .get_signature_status_with_commitment(sig, cfg.commitment)
                    .await;
                ...
                match status {
                    Ok(None) => {
                        ...
                        let blockhash_not_found = rpc_client
                            .is_blockhash_valid(blockhash, cfg.commitment)
                            .await?;
